...
Implement the built-in validation engine in `core/validation/validation.go` with an `Errors` type, a `Validator` struct, and 9 chainable validation methods. All stdlib, no external dependencies.
...
| File | Purpose |
|------|---------|
| `core/validation/validation.go` | Errors type, Validator struct, 9 validation methods |
| `core/validation/validation_test.go` | Tests for all validation rules |
...
None — all Go stdlib:
- `fmt`, `net`, `net/mail`, `regexp`, `strings`, `unicode/utf8`
